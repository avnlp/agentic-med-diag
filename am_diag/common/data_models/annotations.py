...
They are informational markers — the system also supports the legacy `metadata`
dict approach.

Usage::

    from typing import Annotated
    from am_diag.common.data_models import DataPoint, Embeddable, LLMContext, Dedup
...
class _Embeddable:
    ...
    def __repr__(self) -> str:
        return (
            f"Embeddable({self.description!r})" if self.description else "Embeddable()"
        )
...
class _LLMContext:
    ...
    def __repr__(self) -> str:
        return (
            f"LLMContext({self.description!r})" if self.description else "LLMContext()"
        )
...
def Embeddable(  # noqa: N802
    description: str = "Embedded in vector DB for semantic search",
) -> _Embeddable:
...
def LLMContext(  # noqa: N802
    description: str = "Sent to LLM during retrieval",
) -> _LLMContext:
...
def Dedup(  # noqa: N802
    description: str = "Used for entity deduplication",
) -> _Dedup:
